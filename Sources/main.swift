...
routes.add(method: .get, uri: "/") { (request, response) in
    response.setBody(string: "Hello, This is Spark App Studio's Swift Server").completed()
}
...
func returnJSONMessage(message: String, response: HTTPResponse) {
    do {
        ...
        response.setBody(string: "error handling request").completed(status: .internalServerError)
    }
}
